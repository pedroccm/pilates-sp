...
export default function NovoStudioPage() {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      uploadFormData.append('studioSlug', generateSlug(formData.title, formData.neighborhood, formData.city))
                  ...
                  {formData.imageUrl ? (
                    <div className="relative">
                      <img
                        ...
                        alt={formData.title || 'Novo estúdio'}
                        className="w-full h-48 object-cover rounded-lg border border-gray-300"
                      />
                      <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded">
                        {formData.title || 'Novo estúdio'}
                      </div>
                    </div>
                  ) : (
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          if (!formData.title.trim()) {
                            alert('Por favor, preencha o nome do estúdio antes de fazer upload da imagem')
                            return
                          }
                      ...
                      <div className="mt-4">
                        ...
                        {!formData.title.trim() && (
                          <p className="text-xs text-orange-500 mt-1">
                            ⚠️ Preencha o nome do estúdio primeiro
                          </p>
                        )}
                      </div>
                    </label>
