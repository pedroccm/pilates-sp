...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validar tamanho (max 2MB para avatares)
    if (file.size > 2 * 1024 * 1024) {
      return NextResponse.json({ error: 'Arquivo muito grande. Máximo 2MB para avatares' }, { status: 400 })
    }
    ...
    const fileName = `avatars/${timestamp}-${randomString}.${fileExtension}`
